...
def _ensure_optional_deps_importable() -> None:
    ...
    The production code imports `boto3` at module import-time for the S3 handler,
...
def _ensure_datahub_rest_emitter_aliases() -> None:
    """
    Some modules import `DataHubRestEmitter` (capital H) which is not the canonical
    name in `acryl-datahub` (it is typically `DatahubRestEmitter`).
    We alias it for unit tests so imports succeed without touching production code.
    """
    ...
    try:
        ...
        # If datahub isn't available, tests that require it should skip or stub further.
        return
...
_ensure_datahub_rest_emitter_aliases()
